...
export default function GoalProgress({ monthlyTarget, monthPnl, dailyTarget, todayPnl, tradingDaysPassed, className }: Props) {
  ...
  const paceLabel = pace === 'ahead' ? t.goal.ahead : pace === 'on_track' ? t.goal.onTrack : t.goal.behind;
    ...
    <div className={cn("terminal-border rounded-lg p-4 space-y-4", className)}>
      <div className="flex items-center justify-between">
        <h2 className="text-xs font-bold text-foreground flex items-center gap-2">
          <Target className="h-3.5 w-3.5 text-primary" /> {t.goal.monthlyGoal}
        </h2>
        <div className={cn(
      ...
      <div className="space-y-1.5">
        <div className="flex justify-between text-[10px] font-mono text-muted-foreground">
          <span>${monthPnl.toFixed(0)} {t.goal.earned}</span>
          <span>${monthlyTarget.toLocaleString()} {t.goal.target}</span>
        </div>
        <div className="relative h-3 rounded-full bg-muted overflow-hidden">
          <div className="absolute top-0 bottom-0 w-0.5 bg-foreground/30 z-10" style={{ left: `${expectedPct}%` }} />
          <div className={cn("h-full rounded-full transition-all duration-700",
            pace === 'ahead' ? "bg-profit" : pace === 'on_track' ? "bg-primary" : "bg-warning"
          )} style={{ width: `${progressPct}%` }} />
        </div>
        <div className="flex justify-between text-[10px] font-mono">
          <span className="text-muted-foreground">{progressPct.toFixed(0)}% {t.goal.complete}</span>
          <span className="text-muted-foreground">${remaining.toFixed(0)} {t.goal.remaining}</span>
        </div>
      </div>

      <div className="space-y-1.5">
        <div className="flex justify-between text-[10px] font-mono text-muted-foreground">
          <span>{t.goal.todayLabel}: ${todayPnl.toFixed(0)}</span>
          <span>{t.goal.targetLabel}: ${dailyTarget.toFixed(0)}</span>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-[10px] font-mono">
        <div className="bg-accent/30 rounded-md p-2">
          <span className="text-muted-foreground">{t.goal.reqDaily}</span>
          <div className={cn("font-bold", requiredDaily <= dailyTarget * 1.3 ? "text-foreground" : "text-loss")}>
            ${requiredDaily.toFixed(0)}
          </div>
        </div>
        <div className="bg-accent/30 rounded-md p-2">
          <span className="text-muted-foreground">{t.goal.daysLeft}</span>
