  ...
  if (!signal) return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-card border border-border rounded-lg p-6 w-full max-w-sm text-center" onClick={(e) => e.stopPropagation()}>
        <p className="text-sm text-muted-foreground">Señal no encontrada</p>
        <button onClick={onClose} className="mt-3 text-xs text-primary hover:underline">Cerrar</button>
      </div>
    </div>
  );
      ...
      <div className="bg-card border border-border rounded-lg p-6 w-full max-w-md space-y-4 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-foreground">Idea de Trade Original</h2>
          </div>
        ...
        <div className="rounded-md bg-accent/50 p-3 space-y-2 text-xs font-mono">
          {[
            ['Entrada', formatCurrency(signal.entry_price), ''],
            ['Stop Loss', formatCurrency(signal.stop_loss), 'text-loss'],
            ['Take Profit', formatCurrency(signal.take_profit), 'text-profit'],
            ['R/R', signal.risk_reward.toFixed(2), ''],
            ['Confianza', `${signal.confidence}%`, signal.confidence > 70 ? 'text-profit' : ''],
            ['Estrategia', signal.strategy, 'text-primary'],
        ...
        {signal.reasoning && (
          <div>
            <h3 className="text-xs font-bold text-foreground uppercase tracking-wider mb-1 flex items-center gap-1">
              <Target className="h-3 w-3" /> Razonamiento
            </h3>
        ...
        {signal.agent_analysis && (
          <div>
            <h3 className="text-xs font-bold text-foreground uppercase tracking-wider mb-1 flex items-center gap-1">
              <Shield className="h-3 w-3" /> Análisis del Agente
            </h3>
        ...
        <div className="text-[10px] font-mono text-muted-foreground">
          Creada: {new Date(signal.created_at).toLocaleString()}
        </div>
      </div>
    </div>
  );
