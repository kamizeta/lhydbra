  ...
  try {
    ...
    for (let i = startIdx; i < numericBars.length - 1; i++) {
      ...
      for (let j = i + 1; j < Math.min(i + 11, numericBars.length); j++) {
        ...
        // 4. GAP THROUGH STOP
        if (direction === "long") {
          if (bar.low <= sl) { exitPrice = Math.min(sl, bar.open); outcome = "stop_loss"; break; }
          if (bar.high >= tp) { exitPrice = tp; outcome = "take_profit"; break; }
        } else {
          if (bar.high >= sl) { exitPrice = Math.max(sl, bar.open); outcome = "stop_loss"; break; }
          if (bar.low <= tp) { exitPrice = tp; outcome = "take_profit"; break; }
        }
      }

      if (outcome === "open") {
        const lastIdx = Math.min(i + 10, numericBars.length - 1);
        exitPrice = numericBars[lastIdx].close;
        outcome = "timeout";
      }

      // 2. FEES
      const feesPct = FEE_RATE * 2 * 100; // both sides as percentage
      const grossPnlPct = direction === "long"
        ? (exitPrice - entry) / entry * 100
        : (entry - exitPrice) / entry * 100;
      const pnlPct = grossPnlPct - feesPct;
      ...
      trades.push({
        ...
        outcome, fees_pct: +feesPct.toFixed(3),
      });
    }
