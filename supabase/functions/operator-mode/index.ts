...
/**
 * OPERATOR MODE — Unified orchestrator
 * 
 * Pipeline: 
 * 1. Check cooldown & daily cap
 * 2. Generate signals (via signal-engine logic)
 * 3. Size positions using risk model
 * 4. Optionally execute via Alpaca
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!settings) {
      return jsonRes({ error: "User settings not found. Configure your account first." }, 400);
    }
    ...
    const riskPerTrade = Number(settings.risk_per_trade || 1);
    ...
    if (consecutiveLosses >= lossCooldownCount) {
      preflight.push(`🔴 COOLDOWN ACTIVE: ${consecutiveLosses} consecutive losses (limit: ${lossCooldownCount})`);
    }

    if (tradesToday >= maxTradesPerDay) {
      preflight.push(`🔴 DAILY CAP REACHED: ${tradesToday}/${maxTradesPerDay} trades`);
    }

    if (dailyRiskUsed >= maxDailyRisk) {
      preflight.push(`🔴 DAILY RISK EXHAUSTED: ${dailyRiskUsed.toFixed(1)}% / ${maxDailyRisk}%`);
    }
    ...
    // ─── ACTION: run ───
    // Step 1: Generate signals via signal-engine
    ...
    if (signalResult.blocked) {
      return jsonRes({ status: "blocked", ...signalResult });
    }
    ...
    if (signals.length === 0) {
      return jsonRes({
        ...
        message: "No signals passed the quality filters",
    ...
    const sized = signals.map((sig: Record<string, unknown>) => {
      ...
      // Risk per trade: min of configured risk and remaining daily risk
      ...
      // Max single asset check
      const maxSingleAsset = Number(settings.max_single_asset || 25);
      const positionValue = quantity * entryPrice;
      const positionPct = (positionValue / currentCapital) * 100;
      if (positionPct > maxSingleAsset) {
        quantity = Math.floor((currentCapital * maxSingleAsset / 100) / entryPrice);
      }
    ...
    // Step 3: Execute if auto_execute is on
    const execResults: Record<string, unknown>[] = [];
    
    if (autoExecute && action === "run") {
      for (const trade of sized) {
        try {
          ...
          // If order succeeded, create position in DB
          if (orderResult.success) {
            await supabase.from("positions").insert({
              ...
              avg_entry: Number(trade.entry_price),
              ...
              status: "open",
              notes: `Operator Mode auto-execution. Score: ${Number(trade.opportunity_score).toFixed(0)}, R: ${Number(trade.expected_r_multiple).toFixed(1)}`,
            });
          }
        } catch (execErr) {
          execResults.push({
      ...
      const riskUsedNow = sized
        .filter((_: unknown, i: number) => execResults[i]?.success)
        .reduce((s: number, t: Record<string, unknown>) => s + Number(t.risk_pct || 0), 0);

      await supabase
        .from("user_settings")
        .update({
          trades_today: tradesToday + successfulTrades,
          last_trade_date: today,
          daily_risk_used: dailyRiskUsed + riskUsedNow,
        })
        .eq("user_id", user.id);
      ...
      await supabase.from("daily_performance").upsert({
        ...
        trades_opened: tradesToday + successfulTrades,
        risk_used_pct: dailyRiskUsed + riskUsedNow,
      }, { onConflict: "user_id,date" });
    }

    return jsonRes({
      status: autoExecute ? "executed" : "ready_for_approval",
      ...
      execution: autoExecute ? execResults : null,
      daily_summary: {
        trades_today: tradesToday + (autoExecute ? execResults.filter(r => r.success).length : 0),
