...
-- Function: auto-create trade_journal entry when position is closed
...
AS $$
DECLARE
  v_signal trade_signals%ROWTYPE;
...
BEGIN
  ...
  IF NEW.status = 'closed' AND (OLD.status IS NULL OR OLD.status <> 'closed') THEN
    ...
    -- Update position with r_multiple
    NEW.actual_r_multiple := v_r_multiple;

    -- Fetch signal data if linked
    IF NEW.signal_id IS NOT NULL THEN
      SELECT * INTO v_signal FROM trade_signals WHERE id = NEW.signal_id;
    END IF;
...
-- Add unique constraint for strategy_performance upsert
ALTER TABLE public.strategy_performance 
  ADD CONSTRAINT strategy_performance_user_strategy_regime_unique 
...
CREATE TRIGGER on_position_closed
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.handle_position_closed();
